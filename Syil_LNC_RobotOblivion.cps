...
/*----------------------------------------------------------------------------------------------------------
Post setup
  - Set auto and manual tool change options and position values @ line: 263-310
  - If unchecked the post will fall back to the home position in the machine model and then to the post machine configuration 
        For the most realistic machine simulation to use:
          retract: {
            cancelRotationOnRetracting: false, // specifies that rotations (G68) need to be canceled prior to retracting
            methodXY                  : "G53", // will use the post properties tool change location 
            methodZ                   : undefined, // leave undefined so that the post properties drop-down can override 
            useZeroValues             : ["G28"], // do not add G53 as this will cause issues with the toolchange position and machine simulation
            homeXY                    : {onIndexing:false, onToolChange:{axes:[X, Y]}, onProgramEnd:{axes:[X, Y]}}, // homing xy on indexing only possible if retract to clearance plane diabled 
      Setting the home values for each axis in the machine builder is recommended to park the mill at the end of the program
...
vendor = "Robot Oblivion";
vendorUrl = "www.instagram.com/RobotOblivion";
legal = "Copyright (C) 2012-2024 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 45917;

longDescription = "Syil LNC 6800 Post Processor with A-axis and machine simulation by Robot Oblivion based on Scott Moyse's oringal Toolpath.com Syil post.";
...
/*   preloadTool: {
  ...
  useSmoothing: {
    ...
    values     : [
      ...
      {title:"Fine Precision", id:"10"},
      {title:"Precision", id:"8"},
      {title:"Balanced", id:"6"},
      {title:"Semi-rough", id:"4"},
      {title:"Roughing", id:"2"}
    ],
    value      : "-1",
    scope      : "operation", //,"post"
    enable     : "milling",
    disabled   : ["probing" , "drilling"]
  },
  /* precisionLevel: {
  ...
  _1spinOnProbe: {
    title      : "Spin probe on/off",
    description: "Specifies whether spin-on and off cycles are required for the probe",
    group      : "multiAxis",
    type       : "boolean",
    value      : true,
    scope      : "post"
  ...
  probeOnMCode: {
    title      : "M-code Probe On",
    description: "M-code to use to spin on the probe.",
    group      : "multiAxis",
    type       : "integer",
    value      : 10719,
    scope      : "post"
  ...
  probeOffMCode: {
    title      : "M-code Probe off",
    description: "M-code to use to spin off the probe.",
    group      : "multiAxis",
    type       : "integer",
    value      : 10720,
    scope      : "post"
  ...
  safePositionMethod: {
    title      : "Safe Retracts",
    description: "Select your desired Z axis retract method.",
    group      : "homePositions",
    type       : "enum",
    values     : [
      {title:"G28", id:"G28"},
  ...
  retractToClearancePlane: {
    title      : "Clearance height retracts",
    description: "Retracts to the operation clearance height; however, any tool or WCS changes will trigger a full Z-axis retract.",
    group      : "homePositions",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  _2TCposX: {
    ...
    group      : "toolChange",
    type       : "number",
    value      : -390, // Back left corner for syil x7
    scope      : "post"
  },
  _2TCposY: {
    ...
    group      : "toolChange",
    type       : "number",
    value      : 0, // Back left corner for syil x7
    scope      : "post"
  },
  _3forceManualTCPosition: {
    title      : "Change Position Before Manual Toolchange",
    description: "Change the machine position before executing a manual toolchange.",
    group      : "toolChange",
    type       : "boolean",
    value      : false,
    scope      : "post"
  ...
  _4manual_TCposX: {
    title      : "Manual toolchange Position X Axis - Machine Coordinate",
    description: "Machine Coordinate for manual toolchange - X Axis.",
    group      : "toolChange",
    type       : "number",
    value      : 0,
    scope      : "post"
  ...
  _4manual_TCposY: {
    title      : "Manual toolchange Position Y Axis - Machine Coordinate",
    description: "Machine Coordinate for manual toolchange - Y Axis.",
    group      : "toolChange",
    type       : "number",
    value      : 0,
    scope      : "post"
...
groupDefinitions = {
  CustomGcode       : {title:"Custom G-code", description: "G-code to add to the start of the program", collapsed: true, order: 10},
  toolChange        : {title:"Tool change", description: "Setting related to toolchanges", collapsed: true, order: 31},
  multiAxis        : {title:"Probe and 4th axis M-codes ", description: "Settings related to the probe outputs and 4th axis clamp", collapsed: true, order: 34},
};
...
wcsDefinitions = {
  ...
  wcs          : [
    ...
    {name:"Extended", format:"G54 P", range:[1, 106]} //format:"G54 P#" in old post
  ]
};
...
var toolTableFormat = createFormat({decimals:1,type:FORMAT_INTEGER}); //  layout spacer for tool table
...
var settings = {
  ...
  smoothing: {
    ...
    semifinishing         : 6, // semi-finishing level for smoothing in automatic mode
    ...
    thresholdsemiRough    : toPreciseUnit(0.25, MM), // operations with stock/tolerance below that threshold will use simi level in automatic mode
  ...
  retract: {
    cancelRotationOnRetracting: false, // specifies that rotations (G68) need to be canceled prior to retracting
    methodXY                  : "G53", // keeps sim true by working in mech coor. special condition, overwrite retract behavior per axis
    methodZ                   : undefined, // special condition, overwrite retract behavior per axis
    useZeroValues             : ["G28"], // keeps sim true by using zero value for Z moves and different method than XY enter property value id(s) for using "0" value instead of machineConfiguration axes home position values (ie G30 Z0)
  ...
  workPlaneMethod: {
    ...
    //useABCPrepositioning  : true, // position ABC axes prior to tilted workplane blocks KC to test why off
...
function onOpen() {
  ...
  writeBlock(getProperty("_1CustomOnOpenGcode"));
}
...
function setSmoothing(mode) {
  ...
  if (mode) { // enable smoothing
    ...
    writeBlock(gFormat.format(5), "P" + smoothing.level);
...
var macroCall = settings.probing.macroCall; // Moved up so set before initial positioning

function onSection() {
  if(debugByCarnie >= 1 && debugByCarnie <= 2){writeComment("onSection---------v")}; // go to line 55 to set debug level

  var forceSectionRestart = !isFirstSection() && getPreviousSection().isOptional() && !currentSection.isOptional();
  ...
  /* Code below move to end of onSectionEnd
  ...
  Code above move to end of onSectionEnd */
  ...
  // retract Z axis after any optional section and may not be required, as picked up by forceSectionRestart in onSectionEnd, but it adds an extra Z retract at the start of the section after any optional section in case of at machine change of location by operator in manual mode. 
  if ((!isFirstSection() && getPreviousSection().isOptional()) && optionalSection != true ) {
    //state.retractedZ = false // likely not required 
    writeRetract(Z);
  }
  ...
  // Spin on probe
  if (isProbeOperation() && (isFirstSection() || getPreviousSection().getTool().type != TOOL_PROBE) && getProperty("_1spinOnProbe")) {
  ...
  // Force simulation line to patch a bug in machine connection moves when swapping WCS without a tool change in between
  if (!isFirstSection() && newWorkOffset && !insertToolCall) {
    var finalPositionPrevious =  getPreviousSection().getFinalPosition()
    var xfinalPositionPrevious = finalPositionPrevious.getCoordinate(0);
    var yfinalPositionPrevious = finalPositionPrevious.getCoordinate(1);
    machineSimulation({x: xfinalPositionPrevious, y: yfinalPositionPrevious, mode:TOOLCHANGE}) // kc to sort this is now not needed sometimes
  }
  ...
  if(debugByCarnie >= 1 && debugByCarnie <= 2){writeComment("onSection---------^")}; // go to line 55 to set debug level
}
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    ...
    //var preloadTool = getNextTool(tool.number != getFirstTool().number); // preload tools not supported on umbrella changer machines
  ...
  case COMMAND_BREAK_CONTROL:
    writeBlock(mFormat.format(106) + " T" + toolFormat.format(tool.number)+ " D" + xyzFormat.format(tool.diameter) + " E"+ getProperty("_0breakControlError"));
    machineSimulation({x:toPreciseUnit(getProperty("_2TCposX"), MM), y:toPreciseUnit(getProperty("_2TCposY"), MM), coordinates:MACHINE, mode:TOOLCHANGE }); // Machine Simulation to the tool change position
    return;
  case COMMAND_TOOL_MEASURE:
    return;
  case COMMAND_PROBE_ON:
    writeBlock(mFormat.format(getProperty("probeOnMCode"))); // Spin the probe on
    return;
  case COMMAND_PROBE_OFF:
    writeBlock(mFormat.format(getProperty("probeOffMCode"))); // Spin the probe off
    writeBlock(mFormat.format(19)) // Turn probe to the front for faster removal 
    writeBlock(mFormat.format(20)) // unlock spindle orintation 
    return;
  }
...
function onSectionEnd() {
  if(debugByCarnie >= 1 && debugByCarnie <= 2){writeComment("onSectionEnd---v")}; // go to line 55 to set debug level
  ...
  // Spin off probe
  if (isProbeOperation()) {
    if (isLastSection() || getNextSection().getTool().type != TOOL_PROBE && getProperty("_1spinOnProbe")) {
     onCommand(COMMAND_PROBE_OFF)
    }
    /*if (probeVariables.probeAngleMethod != "G68") {
      setProbeAngle(); // Output probe angle rotations if required (may not be required as pulled from old post)
    }*/
  }

  if (!isLastSection()) {
    if (nextSection.getTool().coolant != tool.coolant) {
      setCoolant(COOLANT_OFF);
    }
    if (tool.breakControl && isToolChangeNeeded(nextSection, getProperty("toolAsName") ? "description" : "number")) {
      setCoolant(COOLANT_OFF);
      onCommand(COMMAND_STOP_SPINDLE);
      writeRetract(Z);
    ...
    //Unload manual tool and replace with pocket placeholder or chip fan
    if (tool.manualToolChange && isToolChangeNeeded(nextSection, getProperty("toolAsName") ? "description" : "number")) { 
        ...
        if (getProperty("_3forceManualTCPosition")) {
          ...
          machineSimulation({x:toPreciseUnit(getProperty("_4manual_TCposX"), MM), y:toPreciseUnit(getProperty("_4manual_TCposY"), MM), coordinates:MACHINE}); // , mode:TOOLCHANGE move machineSimulation to a tool change position
        } else {
          if (getSetting("retract.homeXY.onToolChange", false)) {
        ...
        writeComment("Flip to manual, remove T" + tool.number + " and replace with pocket placeholder"); // wording for use with LOADTOOL macro : T" + tool.number + " then cycle start to resume" );
    }
  }
  ...
  //------------ onSection() setup below:
  initializeSmoothing(); // initialize smoothing mode

  if (insertToolCallNext || newWorkOffsetNext || newWorkPlaneNext || smoothing.cancel || state.tcpIsActive) {
    
    // stop spindle before retract during tool change
    if (insertToolCallNext && !isLastSection()) {
      onCommand(COMMAND_STOP_SPINDLE);
    }
    disableLengthCompensation();
    if (getSetting("workPlaneMethod.cancelTiltFirst", false)) {
    ...
    // Retracts between opperations added to this section
    if (!getProperty("retractToClearancePlane") || insertToolCallNext || newWorkOffsetNext){
      writeRetract(Z);
      if (newWorkOffsetNext && !insertToolCallNext) {
      }
    }
    if (isFirstSection() && machineConfiguration.isMultiAxisConfiguration()) {
      setWorkPlane(new Vector(0, 0, 0)); // reset working plane
    ...
    if (insertToolCallNext || smoothing.cancel || state.tcpIsActive) {
      disableLengthCompensation();
      if (smoothing.cancel || insertToolCallNext) {
        setSmoothing(false);
      }
    }
  }
  if(debugByCarnie >= 1 && debugByCarnie <= 2){writeComment("onSectionEnd---^")}; // go to line 55 to set debug level
}
...
function onClose() {
  if(debugByCarnie >= 1 && debugByCarnie <= 2){writeComment("onClose---------v")}; // go to line 55 to set debug level
  ...
  writeln("%");
  writeBlock(getProperty("_2CustomOnCloseGcode"));
  if(debugByCarnie >= 1 && debugByCarnie <= 2){writeComment("onClose---------^")}; // go to line 55 to set debug level
}
...
/*   if (getProperty("retractToClearancePlane")) {
...
function writeToolBlock() {
  ...
  machineSimulation({x:toPreciseUnit(getProperty("_2TCposX"), MM), y:toPreciseUnit(getProperty("_2TCposY"), MM), coordinates:MACHINE, mode:TOOLCHANGE});
}
...
var debugSimulation = debugByCarnie >= 2 && debugByCarnie <= 3 ? true : false; // enable to output debug information for connection move support in the NC program
var TCPON = "TCP ON";
var TCPOFF = "TCP OFF";
var TWPON = "TWP ON";
var TWPOFF = "TWP OFF";
...
function positionABC(abc, force) {
  ...
  if (a || b || c) {
    //writeRetract(Z); // *** retract removed
...
function writeToolCall(tool, insertToolCall) {
  ...
  writeStartBlocks(insertToolCall, function () {
    writeRetract(Z);
    if (getProperty("_1forceTCPosition", false)) {
      forceModals(gMotionModal);
      writeBlock(gAbsIncModal.format(90), gFormat.format(53), gMotionModal.format(0), xOutput.format(getProperty("_2TCposX")), yOutput.format(getProperty("_2TCposY")));
      machineSimulation({x:toPreciseUnit(getProperty("_2TCposX"), MM), y:toPreciseUnit(getProperty("_2TCposY"), MM), coordinates:MACHINE, mode:TOOLCHANGE}); //  move machineSimulation to a tool change position
    } else if (getSetting("retract.homeXY.onToolChange", false)) {
      writeRetract(settings.retract.homeXY.onToolChange);
    }
    ...
    if (tool.manualToolChange) {
      onCommand(COMMAND_LOAD_TOOL); //tool change to bring "pocket placeholder" into spindle
      if (getProperty("_3forceManualTCPosition", false)) {
        forceModals(gMotionModal);
        writeBlock(gAbsIncModal.format(90), gFormat.format(53), gMotionModal.format(0), xOutput.format(getProperty("_4manual_TCposX")), yOutput.format(getProperty("_4manual_TCposY")));
        machineSimulation({x:toPreciseUnit(getProperty("_4manual_TCposX"), MM), y:toPreciseUnit(getProperty("_4manual_TCposY"), MM), coordinates:MACHINE}); // , mode:TOOLCHANGE move machineSimulation to a tool change position
      } else if (getSetting("retract.homeXY.onToolChange", false)) {
    ...
    if (false) { // set to true to use the post kernel version of the tool list
      ...
      if (tools.getNumberOfTools() > 0) {
        for (var i = 0; i < tools.getNumberOfTools(); ++i) {
          ...
          toolTableFormat.format(tool.diameter) +"D " +// " " +
          toolTableFormat.format(tool.bodyLength) + localize("-BH ") +
           toolTableFormat.format(tool.cornerRadius) + localize("-CR ") ;

          if (zRanges[tool.number]) {
            comment += localize("Zmin") + "=" + toolTableFormat.format(zRanges[tool.number].getMinimum())+ " ";
          }
...
// below pulled from old post but not sure of best location
...
// above pulled from old post but not sure of best location 
...
function writeRetract() {
  ...
  if (retract && retract.words.length > 0) {
    ...
    for (var i in retract.words) {
      ...
      /*    simulation method to only display from previous method 
      var retractPositionZSim = state.retractedZ ==  false ? currentSection.getFinalPosition().getZ() : retract.positions.z ; //
      var coordinatesZSim = state.retractedZ == false ? WORK : MACHINE;   
      xSim = retract.singleLine || words.indexOf("X") !== -1 ? retract.positions.x : undefined;
      ySim = retract.singleLine || words.indexOf("Y") !== -1 ? retract.positions.y : undefined;
      zSim = retract.singleLine || words.indexOf("Z") !== -1 ? retractPositionZSim : undefined;
      // Only call machineSimulation for X and Y if either has a value
      if (xSim !== undefined || ySim !== undefined) {
        machineSimulation({
          x: xSim,
          y: ySim,
          coordinates: MACHINE
      ...
      // Only call machineSimulation for Z if it has a value and switch between coordinates methods  
      if (zSim !== undefined) {
        machineSimulation({
          z: zSim,
          coordinates: coordinatesZSim,
        });
      } 
      */
  ...
  writeStartBlocks(isRequired, function() {
    ...
    if (currentSection.isMultiAxis() && getSetting("workPlaneMethod.prepositionWithTWP", true) && getSetting("workPlaneMethod.useTiltedWorkplane", false) &&
      ...
      if (machineConfiguration.isHeadConfiguration()) {
        ...
        // non-protected move for the probe is required to ensure the correct H value is issued. A tall clearance height set in Fusion is recommended
        ...
        machineSimulation({x:position.x, y:position.y, mode:TOOLCHANGE}); // TOOLCHANGE added to patch simulation bug when swapping WCS without a tool change between
...
function writeProbingToolpathInformation(cycleDepth) {
  ...
  //writeln(formatLocalVariable("DPRNT[TOOLPATHID*", 1, "[35]]")); KC removed
  if (isInspectionOperation()) {
    ...
    //writeln(formatLocalVariable("DPRNT[CYCLEDEPTH*", 2, macroRoundingFormat + "]"));  KC removed
  }
}
// <<<<< INCLUDED FROM include_files/commonInspectionFunctions_fanuc.cpi
...
function writeProbeCycle(cycle, x, y, z, P, F) {
  ...
  if (isProbeOperation()) {
    ...
    if (printProbeResults()) {
      //writeProbingToolpathInformation(z - cycle.depth + tool.diameter / 2); //#20 #21
      //inspectionWriteCADTransform(); // #20 21 22 23 24 25
      //inspectionWriteWorkplaneTransform(); 20 21 22 and dprnt line
      if (typeof inspectionWriteVariables == "function") {
        inspectionVariables.pointNumber += 1;
      }
    }
    //protectedProbeMove(cycle, x, y, z); // removed as added extra unrequied moves
  }

  //var macroCall = settings.probing.macroCall; // moved up above onSection()
  switch (cycleType) {
  case "probing-x":
    //forceXYZ();
    ...
    //EXPECTED_Y = "Y" + xyzFormat.format(y + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2)); // KC to sort
  ...
  case "probing-y":
    ...
    WCS_CODE   = getProbingArguments(cycle, probeWorkOffsetCode);
    EXPECTED_Y = "Y" + xyzFormat.format(y + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2)); // yOutput.format
    ...
    break;
  case "probing-z":
    //forceXYZ(); // KC adds go to zero probe pos moves before but would need to drop out entry moves above
  ...
  case "probing-xy-circular-boss":
    ...
    if (WCS_CODE[7] === "I1.") {
      writeBlock(openString);
      writeBlock("PRINT[\"Measured circular boss diameter " + (unit==MM?"mm":"in") + " X: @998\"]");
      writeBlock("PRINT[\"Measured circular boss diameter " + (unit==MM?"mm":"in") + " Y: @999\"]");
  ...
  case "probing-xy-circular-hole":
    ...
    if (WCS_CODE[7] === "I1.") {
      writeBlock(openString);
      writeBlock("PRINT[\"Measured bore diameter " + (unit==MM?"mm":"IN") + " X: @998\"]");
      writeBlock("PRINT[\"Measured bore diameter " + (unit==MM?"mm":"IN") + " Y: @999\"]");
  ...
  case "probing-xy-rectangular-hole":
    ...
    if (WCS_CODE[7] === "I1.") {
      writeBlock(openString);
      writeBlock("PRINT[\"Measured pocket length " + (unit==MM?"mm":"in") + " X: @998\"]");
      writeBlock("PRINT[\"Measured pocket width " + (unit==MM?"mm":"in") + " Y: @999\"]");
      writeBlock("CLOSE[]");
    }
    break;
  case "probing-xy-rectangular-boss":
    //forceXYZ(); // may not be requied as alread at location in initial move above KC to sort
    protectedProbeMove(cycle, x, y, z);
    ...
    if (WCS_CODE[7] === "I1.") {
    writeBlock(openString);
      writeBlock("PRINT[\"Measured rectangular boss length " + (unit==MM?"mm":"in") + " X: @998\"]");
      writeBlock("PRINT[\"Measured rectangular boss width " + (unit==MM?"mm":"in") + " Y: @999\"]");
      writeBlock("CLOSE[]");
    }
    break;
...
function getProbingArguments(cycle, probeWorkOffsetCode) { // Old Toolpath Syil function
  ...
  if (probeOutputWorkOffset <= 6) {
    ...
    PROBE_ARGS = "A54.0" + WCS_NUM;
    }
  }

  var PROBE_OVERRIDE_ARGS = "";
  if (currentWorkOffset <= 6) {
    ...
    if (WCS_NUM >= 10) {
      ...
      PROBE_OVERRIDE_ARGS = "B54.0" + WCS_NUM;
    }
  }
  ...
  return [
    ...
    ("Q" + (getProperty("probeScreenPrint") ? 1 : 0))
  ];
}
// <<<<< INCLUDED FROM include_files/getProbingArguments_renishaw.cpi
...
function setProbeAngle() {
  if (probeVariables.outputRotationCodes) {
    ...
    //var probeOutputWorkOffset = currentSection.probeWorkOffset; // called above function onParameter() KC
